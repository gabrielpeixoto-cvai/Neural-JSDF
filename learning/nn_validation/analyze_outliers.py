...
q_min = (
    torch.tensor(
        [-2.8973, -1.7628, -2.8973, -3.0718, -2.8973, -0.0175, -2.8973, -1, -1, -0.2]
    )
    .cpu()
    .numpy()
)
q_max = (
    torch.tensor([2.8973, 1.7628, 2.8973, -0.0698, 2.8973, 3.7525, 2.8973, 1, 1, 1.3])
    .cpu()
    .numpy()
)
q_span = q_max - q_min
...
for i in range(9):
    print("Link %d" % (i + 1))
    data = np.loadtxt("outliers_link%d.csv" % (i + 1), delimiter=",")
    N = data.shape[0]
    input = data[:N, :10]
