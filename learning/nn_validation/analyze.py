...
for i in range(n_links):
    ...
    for j in range(len(bins) - 1):
        ...
        bin_outliers_raw = np.concatenate(
            [
                inputs_lcl[outlier_indices, :],
                nn_dist_lcl[outlier_indices, :],
                mesh_dist_lcl[outlier_indices, :],
            ],
            axis=1,
        )
        ...
        if len(outlier_indices) > n_outliers:
            ...
            subsample_indices = outlier_indices_sorted[::-1][::step][
                : (n_outliers // 2)
            ]
            subsample_indices = np.append(
                subsample_indices, outlier_indices_sorted[::-1][:100:10]
            )
    ...
    ax.boxplot(
        bin_errors,
        positions=np.array(bins[:-1]),
        widths=bin_w - 5,
        showmeans=False,
        flierprops={"marker": "+", "color": "gray", "alpha": 0.5},
        showfliers=False,
    )
    # Overlay outliers
...
for i, data in enumerate(all_outliers):
    np.savetxt("outliers_link%d.csv" % (i + 1), data, delimiter=",", fmt="%4.3f")
